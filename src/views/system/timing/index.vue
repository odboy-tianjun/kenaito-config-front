  ...
  <div class="app-container">
    ...
    <div class="head-container">
      <div v-if="crud.props.searchToggle">
        ...
        <el-select v-model="query.moduleName" clearable filterable size="small" placeholder="请选择模块名称" style="width: 200px;" class="filter-item">
          <el-option
      ...
      <Log ref="log" />
      <a href="https://www.pppet.net/" style="color: blue;font-size: 12px" target="_blank">Cron表达式在线生成</a>
    </div>
    <!--Form表单-->
    <el-dialog :close-on-click-modal="false" :before-close="crud.cancelCU" :visible.sync="crud.status.cu > 0" :title="crud.status.title" append-to-body width="730px">
      <el-form ref="form" :inline="true" :model="form" :rules="rules" size="small" label-width="100px">
        <el-form-item label="所属模块">
          <el-select v-model="form.moduleName" clearable filterable allow-create placeholder="请选择模块名称" style="width: 220px">
            <el-option
              v-for="item in moduleNameOptions"
              :key="item.value"
              :label="item.label"
    ...
    <el-table ref="table" v-loading="crud.loading" :data="crud.data" style="width: 100%;" @selection-change="crud.selectionChangeHandler">
      ...
      <el-table-column :show-overflow-tooltip="true" prop="moduleName" label="所属模块" />
...
export default {
  ...
  watch: {
    ...
    // 当对象crud中的data发生变化时
    'crud.data'(newVal, oldVal) {
      if (newVal !== oldVal) {
        crudJob.queryModules().then((data) => {
          this.moduleNameOptions = data || []
        })
      }
    }
  },
